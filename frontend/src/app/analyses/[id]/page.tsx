  ...
  const fetchData = async () => {
    try {
      const [
        ...
        fetch(
          `${API_BASE_URL}/api/analysis/${analysisId}/metrics`
        ),
        fetch(
          `${API_BASE_URL}/api/analysis/${analysisId}/alerts`
        )
      ]);
      ...
      if (analysisRes.ok) {
        ...
        const videoRes = await fetch(
          `${API_BASE_URL}/api/videos/${analysisData.video_id}`
        );
  ...
  const handleAcknowledgeAlert = async (
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/analysis/${analysisId}/alerts/${alertId}/acknowledge`,
        {
          method: 'PUT'
        }
      );
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
            ...
            <div className="text-3xl font-bold text-white">
              {analysis.max_density !== null &&
                analysis.max_density !== undefined
                ? analysis.max_density.toFixed(3)
                : '-'}
            </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700">
            ...
            <div className="text-3xl font-bold text-white">
              {analysis.max_risk_score !== null &&
                analysis.max_risk_score !== undefined
                ? analysis.max_risk_score.toFixed(1)
                : '-'}
            </div>

            <div className="text-slate-500 text-sm mt-1">
              Avg Flow:{' '}
              {analysis.avg_flow_rate?.toFixed(3) ?? '-'}
            </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-slate-700 mb-8">
            ...
            <div className="bg-black rounded-lg overflow-hidden">
              ...
              <video
                ...
                muted
                className="w-full"
                src={`${API_BASE_URL}/api/videos/${video.id}/stream`}
              >
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 text-white">
                      ...
                      <span>
                        Density: {metric.density.toFixed(3)}
                      </span>

                      <span>
                        Flow: {metric.flow_rate?.toFixed(3) ?? 'N/A'}
                      </span>

                      <span>
                        Velocity: {metric.avg_velocity?.toFixed(2) ?? 'N/A'}
                      </span>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-300">
                  Overall Bottleneck Risk
                </span>

                <span
                  className={`font-semibold ${getRiskLevelColor(
                    analysis.max_risk_score >= 76
                      ? 'critical'
                      : analysis.max_risk_score >= 56
                        ? 'high'
                        : analysis.max_risk_score >= 31
                          ? 'warning'
                          : 'safe'
                  )}`}
                >
              ...
              <div className="text-slate-400 text-sm">
                ...
                Bottleneck risk is calculated from density,
                flow consistency, and velocity.
                High density combined with low flow
                consistency or low velocity indicates
                potential bottleneck conditions.
